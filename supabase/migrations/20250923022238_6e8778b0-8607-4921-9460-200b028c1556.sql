...
-- Baseline risk scores (use INSERT or UPDATE pattern since there's no unique constraint)
...
SELECT * FROM (
  VALUES 
    ...
    ('u_105', 2.0, now()), 
    ('u_106', 9.5, now())
) AS new_scores(person_id, score, calculated_at)
WHERE NOT EXISTS (
  SELECT 1 FROM risk_score WHERE risk_score.person_id = new_scores.person_id
);
