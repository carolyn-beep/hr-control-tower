-- Drop the view that's blocking the schema change
DROP VIEW IF EXISTS public.v_release_open;
...
CREATE VIEW public.v_release_open WITH (security_invoker = true) AS
SELECT rc.id, p.name, p.email, rc.opened_at, rc.reason, rc.status, rc.risk_score
FROM release_case rc 
