...
-- Baseline risk scores (keeping within 0-10 range)
insert into risk_score (person_id, score, calculated_at) values
  ('u_101', 4.0, now()), ('u_102', 8.5, now()), ('u_103', 3.0, now()),
  ('u_104', 7.0, now()), ('u_105', 2.0, now()), ('u_106', 9.5, now());

-- Clear any old demo spam for these ids (optional)
delete from signal where id::text like 'seed_%';

-- Fresh signals (generate proper UUIDs)
insert into signal (person_id, ts, level, reason, score_delta) values
  ('u_101', now() - interval '50 minutes', 'warn',     'PR review latency above target', 4),
  ('u_102', now() - interval '2 hours',    'risk',     'Low PR throughput + high reopen rate', 10),
  ('u_103', now() - interval '1 day',      'info',     'On-call recovery within SLO', -2),
  ('u_104', now() - interval '3 hours',    'risk',     'Frontend build failures this sprint', 8),
  ('u_105', now() - interval '30 minutes', 'warn',     'Onboarding task overdue (>24h)', 5),
  ('u_106', now() - interval '4 hours',    'critical', 'Sustained PR shortfall (2 wks)', 15);
